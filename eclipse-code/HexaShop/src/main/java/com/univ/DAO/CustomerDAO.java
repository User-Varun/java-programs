...
public class CustomerDAO {
	...
	public int insert(CustomerDTO dto) {
		...
		try {
		 ...
		 PreparedStatement ps =	con.prepareStatement("insert into customer(cname , cadd , email , mob , unm , pw) values(? , ? , ? ,? ,? , ?)");
	...
	public int uploadFile(String fnm , String price , String type , InputStream in ) {
		...
		try {
			...
			PreparedStatement ps = con1.prepareStatement("insert into imagedata(imgname ,imgprice ,imgtype) values(? , ? ,? )");
